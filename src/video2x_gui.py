                ...
                # determine file type
                    ...
                    try:
                        input_file_mime_type = magic.from_file(
                            str(file_path.absolute()), mime=True
                        )
...
class Video2XMainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        QShortcut(
            QKeySequence(Qt.CTRL + Qt.SHIFT + Qt.Key_I), self, self.select_input_folder
        )
        QShortcut(
            QKeySequence(Qt.CTRL + Qt.SHIFT + Qt.Key_O), self, self.select_output_folder
        )
        ...
        self.input_table_view.horizontalHeader().setSectionResizeMode(
            0, QHeaderView.Stretch
        )

        # input table buttons
        self.input_select_file_button = self.findChild(
            QPushButton, "inputSelectFileButton"
        )
        self.input_select_file_button.clicked.connect(self.select_input_file)
        self.input_select_folder_button = self.findChild(
            QPushButton, "inputSelectFolderButton"
        )
        self.input_select_folder_button.clicked.connect(self.select_input_folder)
        self.input_delete_selected_button = self.findChild(
            QPushButton, "inputDeleteSelectedButton"
        )
        self.input_delete_selected_button.clicked.connect(
            self.input_table_delete_selected
        )
        ...
        self.output_select_file_button = self.findChild(
            QPushButton, "outputSelectFileButton"
        )
        self.output_select_file_button.clicked.connect(self.select_output_file)
        self.output_select_folder_button = self.findChild(
            QPushButton, "outputSelectFolderButton"
        )
        ...
        self.cache_select_folder_button = self.findChild(
            QPushButton, "cacheSelectFolderButton"
        )
        ...
        self.scale_ratio_double_spin_box = self.findChild(
            QDoubleSpinBox, "scaleRatioDoubleSpinBox"
        )
        self.output_width_spin_box = self.findChild(QSpinBox, "outputWidthSpinBox")
        self.output_width_spin_box.valueChanged.connect(
            self.mutually_exclude_scale_ratio_resolution
        )
        self.output_height_spin_box = self.findChild(QSpinBox, "outputHeightSpinBox")
        self.output_height_spin_box.valueChanged.connect(
            self.mutually_exclude_scale_ratio_resolution
        )
        self.output_file_name_format_string_line_edit = self.findChild(
            QLineEdit, "outputFileNameFormatStringLineEdit"
        )
        self.image_output_extension_line_edit = self.findChild(
            QLineEdit, "imageOutputExtensionLineEdit"
        )
        self.video_output_extension_line_edit = self.findChild(
            QLineEdit, "videoOutputExtensionLineEdit"
        )
        self.preserve_frames_check_box = self.findChild(
            QCheckBox, "preserveFramesCheckBox"
        )

        # frame preview
        self.frame_preview_show_preview_check_box = self.findChild(
            QCheckBox, "framePreviewShowPreviewCheckBox"
        )
        self.frame_preview_keep_aspect_ratio_check_box = self.findChild(
            QCheckBox, "framePreviewKeepAspectRatioCheckBox"
        )
        self.frame_preview_label = self.findChild(QLabel, "framePreviewLabel")

        # currently processing
        self.currently_processing_label = self.findChild(
            QLabel, "currentlyProcessingLabel"
        )
        ...
        # driver settings
        # waifu2x-caffe
        self.waifu2x_caffe_path_line_edit = self.findChild(
            QLineEdit, "waifu2xCaffePathLineEdit"
        )
        self.enable_line_edit_file_drop(self.waifu2x_caffe_path_line_edit)
        self.waifu2x_caffe_path_select_button = self.findChild(
            QPushButton, "waifu2xCaffePathSelectButton"
        )
        self.waifu2x_caffe_path_select_button.clicked.connect(
            lambda: self.select_driver_binary_path(self.waifu2x_caffe_path_line_edit)
        )
        self.waifu2x_caffe_mode_combo_box = self.findChild(
            QComboBox, "waifu2xCaffeModeComboBox"
        )
        self.waifu2x_caffe_noise_level_spin_box = self.findChild(
            QSpinBox, "waifu2xCaffeNoiseLevelSpinBox"
        )
        self.waifu2x_caffe_process_combo_box = self.findChild(
            QComboBox, "waifu2xCaffeProcessComboBox"
        )
        self.waifu2x_caffe_model_combobox = self.findChild(
            QComboBox, "waifu2xCaffeModelComboBox"
        )
        self.waifu2x_caffe_crop_size_spin_box = self.findChild(
            QSpinBox, "waifu2xCaffeCropSizeSpinBox"
        )
        self.waifu2x_caffe_output_quality_spin_box = self.findChild(
            QSpinBox, "waifu2xCaffeOutputQualitySpinBox"
        )
        self.waifu2x_caffe_output_depth_spin_box = self.findChild(
            QSpinBox, "waifu2xCaffeOutputDepthSpinBox"
        )
        self.waifu2x_caffe_batch_size_spin_box = self.findChild(
            QSpinBox, "waifu2xCaffeBatchSizeSpinBox"
        )
        self.waifu2x_caffe_gpu_spin_box = self.findChild(
            QSpinBox, "waifu2xCaffeGpuSpinBox"
        )
        self.waifu2x_caffe_tta_check_box = self.findChild(
            QCheckBox, "waifu2xCaffeTtaCheckBox"
        )

        # waifu2x-converter-cpp
        self.waifu2x_converter_cpp_path_line_edit = self.findChild(
            QLineEdit, "waifu2xConverterCppPathLineEdit"
        )
        self.enable_line_edit_file_drop(self.waifu2x_converter_cpp_path_line_edit)
        self.waifu2x_converter_cpp_path_edit_button = self.findChild(
            QPushButton, "waifu2xConverterCppPathSelectButton"
        )
        self.waifu2x_converter_cpp_path_edit_button.clicked.connect(
            lambda: self.select_driver_binary_path(
                self.waifu2x_converter_cpp_path_line_edit
            )
        )
        self.waifu2x_converter_cpp_png_compression_spin_box = self.findChild(
            QSpinBox, "waifu2xConverterCppPngCompressionSpinBox"
        )
        self.waifu2x_converter_cpp_image_quality_spin_box = self.findChild(
            QSpinBox, "waifu2xConverterCppImageQualitySpinBox"
        )
        self.waifu2x_converter_cpp_block_size_spin_box = self.findChild(
            QSpinBox, "waifu2xConverterCppBlockSizeSpinBox"
        )
        self.waifu2x_converter_cpp_processor_spin_box = self.findChild(
            QSpinBox, "waifu2xConverterCppProcessorSpinBox"
        )
        self.waifu2x_converter_cpp_model_combo_box = self.findChild(
            QComboBox, "waifu2xConverterCppModelComboBox"
        )
        self.waifu2x_converter_cpp_noise_level_spin_box = self.findChild(
            QSpinBox, "waifu2xConverterCppNoiseLevelSpinBox"
        )
        self.waifu2x_converter_cpp_mode_combo_box = self.findChild(
            QComboBox, "waifu2xConverterCppModeComboBox"
        )
        self.waifu2x_converter_cpp_log_level_spin_box = self.findChild(
            QSpinBox, "waifu2xConverterCppLogLevelSpinBox"
        )
        self.waifu2x_converter_cpp_disable_gpu_check_box = self.findChild(
            QCheckBox, "waifu2xConverterCppDisableGpuCheckBox"
        )
        self.waifu2x_converter_cpp_force_opencl_check_box = self.findChild(
            QCheckBox, "waifu2xConverterCppForceOpenclCheckBox"
        )
        self.waifu2x_converter_cpp_tta_check_box = self.findChild(
            QCheckBox, "waifu2xConverterCppTtaCheckBox"
        )

        # waifu2x-ncnn-vulkan
        self.waifu2x_ncnn_vulkan_path_line_edit = self.findChild(
            QLineEdit, "waifu2xNcnnVulkanPathLineEdit"
        )
        self.enable_line_edit_file_drop(self.waifu2x_ncnn_vulkan_path_line_edit)
        self.waifu2x_ncnn_vulkan_path_select_button = self.findChild(
            QPushButton, "waifu2xNcnnVulkanPathSelectButton"
        )
        self.waifu2x_ncnn_vulkan_path_select_button.clicked.connect(
            lambda: self.select_driver_binary_path(
                self.waifu2x_ncnn_vulkan_path_line_edit
            )
        )
        self.waifu2x_ncnn_vulkan_noise_level_spin_box = self.findChild(
            QSpinBox, "waifu2xNcnnVulkanNoiseLevelSpinBox"
        )
        self.waifu2x_ncnn_vulkan_tile_size_spin_box = self.findChild(
            QSpinBox, "waifu2xNcnnVulkanTileSizeSpinBox"
        )
        self.waifu2x_ncnn_vulkan_model_combo_box = self.findChild(
            QComboBox, "waifu2xNcnnVulkanModelComboBox"
        )
        self.waifu2x_ncnn_vulkan_gpu_id_spin_box = self.findChild(
            QSpinBox, "waifu2xNcnnVulkanGpuIdSpinBox"
        )
        self.waifu2x_ncnn_vulkan_jobs_line_edit = self.findChild(
            QLineEdit, "waifu2xNcnnVulkanJobsLineEdit"
        )
        self.waifu2x_ncnn_vulkan_tta_check_box = self.findChild(
            QCheckBox, "waifu2xNcnnVulkanTtaCheckBox"
        )

        # srmd-ncnn-vulkan
        self.srmd_ncnn_vulkan_path_line_edit = self.findChild(
            QLineEdit, "srmdNcnnVulkanPathLineEdit"
        )
        self.enable_line_edit_file_drop(self.srmd_ncnn_vulkan_path_line_edit)
        self.srmd_ncnn_vulkan_path_select_button = self.findChild(
            QPushButton, "srmdNcnnVulkanPathSelectButton"
        )
        self.srmd_ncnn_vulkan_path_select_button.clicked.connect(
            lambda: self.select_driver_binary_path(self.srmd_ncnn_vulkan_path_line_edit)
        )
        self.srmd_ncnn_vulkan_noise_level_spin_box = self.findChild(
            QSpinBox, "srmdNcnnVulkanNoiseLevelSpinBox"
        )
        self.srmd_ncnn_vulkan_tile_size_spin_box = self.findChild(
            QSpinBox, "srmdNcnnVulkanTileSizeSpinBox"
        )
        self.srmd_ncnn_vulkan_model_combo_box = self.findChild(
            QComboBox, "srmdNcnnVulkanModelComboBox"
        )
        self.srmd_ncnn_vulkan_gpu_id_spin_box = self.findChild(
            QSpinBox, "srmdNcnnVulkanGpuIdSpinBox"
        )
        self.srmd_ncnn_vulkan_jobs_line_edit = self.findChild(
            QLineEdit, "srmdNcnnVulkanJobsLineEdit"
        )
        self.srmd_ncnn_vulkan_tta_check_box = self.findChild(
            QCheckBox, "srmdNcnnVulkanTtaCheckBox"
        )

        # realsr-ncnn-vulkan
        self.realsr_ncnn_vulkan_path_line_edit = self.findChild(
            QLineEdit, "realsrNcnnVulkanPathLineEdit"
        )
        self.enable_line_edit_file_drop(self.realsr_ncnn_vulkan_path_line_edit)
        self.realsr_ncnn_vulkan_path_select_button = self.findChild(
            QPushButton, "realsrNcnnVulkanPathSelectButton"
        )
        self.realsr_ncnn_vulkan_path_select_button.clicked.connect(
            lambda: self.select_driver_binary_path(
                self.realsr_ncnn_vulkan_path_line_edit
            )
        )
        self.realsr_ncnn_vulkan_tile_size_spin_box = self.findChild(
            QSpinBox, "realsrNcnnVulkanTileSizeSpinBox"
        )
        self.realsr_ncnn_vulkan_model_combo_box = self.findChild(
            QComboBox, "realsrNcnnVulkanModelComboBox"
        )
        self.realsr_ncnn_vulkan_gpu_id_spin_box = self.findChild(
            QSpinBox, "realsrNcnnVulkanGpuIdSpinBox"
        )
        self.realsr_ncnn_vulkan_jobs_line_edit = self.findChild(
            QLineEdit, "realsrNcnnVulkanJobsLineEdit"
        )
        self.realsr_ncnn_vulkan_tta_check_box = self.findChild(
            QCheckBox, "realsrNcnnVulkanTtaCheckBox"
        )

        # anime4k
        self.anime4kcpp_path_line_edit = self.findChild(
            QLineEdit, "anime4kCppPathLineEdit"
        )
        self.enable_line_edit_file_drop(self.anime4kcpp_path_line_edit)
        self.anime4kcpp_path_select_button = self.findChild(
            QPushButton, "anime4kCppPathSelectButton"
        )
        self.anime4kcpp_path_select_button.clicked.connect(
            lambda: self.select_driver_binary_path(self.anime4kcpp_path_line_edit)
        )
        self.anime4kcpp_passes_spin_box = self.findChild(
            QSpinBox, "anime4kCppPassesSpinBox"
        )
        self.anime4kcpp_push_color_count_spin_box = self.findChild(
            QSpinBox, "anime4kCppPushColorCountSpinBox"
        )
        self.anime4kcpp_strength_color_spin_box = self.findChild(
            QDoubleSpinBox, "anime4kCppStrengthColorSpinBox"
        )
        self.anime4kcpp_strength_gradient_spin_box = self.findChild(
            QDoubleSpinBox, "anime4kCppStrengthGradientSpinBox"
        )
        self.anime4kcpp_threads_spin_box = self.findChild(
            QSpinBox, "anime4kCppThreadsSpinBox"
        )
        self.anime4kcpp_pre_filters_spin_box = self.findChild(
            QSpinBox, "anime4kCppPreFiltersSpinBox"
        )
        self.anime4kcpp_post_filters_spin_box = self.findChild(
            QSpinBox, "anime4kCppPostFiltersSpinBox"
        )
        self.anime4kcpp_platform_id_spin_box = self.findChild(
            QSpinBox, "anime4kCppPlatformIdSpinBox"
        )
        self.anime4kcpp_device_id_spin_box = self.findChild(
            QSpinBox, "anime4kCppDeviceIdSpinBox"
        )
        self.anime4kcpp_codec_combo_box = self.findChild(
            QComboBox, "anime4kCppCodecComboBox"
        )
        self.anime4kcpp_fast_mode_check_box = self.findChild(
            QCheckBox, "anime4kCppFastModeCheckBox"
        )
        self.anime4kcpp_pre_processing_check_box = self.findChild(
            QCheckBox, "anime4kCppPreProcessingCheckBox"
        )
        self.anime4kcpp_post_processing_check_box = self.findChild(
            QCheckBox, "anime4kCppPostProcessingCheckBox"
        )
        self.anime4kcpp_gpu_mode_check_box = self.findChild(
            QCheckBox, "anime4kCppGpuModeCheckBox"
        )
        self.anime4kcpp_cnn_mode_check_box = self.findChild(
            QCheckBox, "anime4kCppCnnModeCheckBox"
        )
        self.anime4kcpp_hdn_check_box = self.findChild(
            QCheckBox, "anime4kCppHdnCheckBox"
        )
        self.anime4kcpp_hdn_level_spin_box = self.findChild(
            QSpinBox, "anime4kCppHdnLevelSpinBox"
        )
        self.anime4kcpp_force_fps_double_spin_box = self.findChild(
            QDoubleSpinBox, "anime4kCppForceFpsDoubleSpinBox"
        )
        self.anime4kcpp_disable_progress_check_box = self.findChild(
            QCheckBox, "anime4kCppDisableProgressCheckBox"
        )
        ...
        # migrate_streams
        self.ffmpeg_migrate_streams_output_options_mapping_video_check_box_check_box = (
            self.findChild(
                QCheckBox, "ffmpegMigrateStreamsOutputOptionsMappingVideoCheckBox"
            )
        )
        self.ffmpeg_migrate_streams_output_options_mapping_audio_check_box_check_box = (
            self.findChild(
                QCheckBox, "ffmpegMigrateStreamsOutputOptionsMappingAudioCheckBox"
            )
        )
        self.ffmpeg_migrate_streams_output_options_mapping_subtitle_check_box_check_box = self.findChild(
            QCheckBox, "ffmpegMigrateStreamsOutputOptionsMappingSubtitleCheckBox"
        )
        self.ffmpeg_migrate_streams_output_options_mapping_data_check_box_check_box = (
            self.findChild(
                QCheckBox, "ffmpegMigrateStreamsOutputOptionsMappingDataCheckBox"
            )
        )
        self.ffmpeg_migrate_streams_output_options_mapping_font_check_box_check_box = (
            self.findChild(
                QCheckBox, "ffmpegMigrateStreamsOutputOptionsMappingFontCheckBox"
            )
        )
        self.ffmpeg_migrate_streams_output_options_pixel_format_line_edit = (
            self.findChild(
                QLineEdit, "ffmpegMigrateStreamsOutputOptionsPixelFormatLineEdit"
            )
        )
        self.ffmpeg_migrate_streams_output_options_frame_interpolation_spin_box = (
            self.findChild(
                QSpinBox, "ffmpegMigrateStreamsOutputOptionsFrameInterpolationSpinBox"
            )
        )
        self.ffmpeg_migrate_streams_output_options_frame_interpolation_spin_box.valueChanged.connect(
            self.mutually_exclude_frame_interpolation_stream_copy
        )
        self.ffmpeg_migrate_streams_output_options_frame_interpolation_spin_box.textChanged.connect(
            self.mutually_exclude_frame_interpolation_stream_copy
        )
        self.ffmpeg_migrate_streams_output_options_copy_streams_check_box = (
            self.findChild(
                QCheckBox, "ffmpegMigrateStreamsOutputOptionsCopyStreamsCheckBox"
            )
        )
        self.ffmpeg_migrate_streams_output_options_copy_known_metadata_tags_check_box = self.findChild(
            QCheckBox, "ffmpegMigrateStreamsOutputOptionsCopyKnownMetadataTagsCheckBox"
        )
        self.ffmpeg_migrate_streams_output_options_copy_arbitrary_metadata_tags_check_box = self.findChild(
            QCheckBox,
            "ffmpegMigrateStreamsOutputOptionsCopyArbitraryMetadataTagsCheckBox",
        )
    ...
    def dropEvent(self, event):
        ...
        for path in input_paths:
            if (path.is_file() or path.is_dir()) and not self.input_table_path_exists(
                path
            ):
    ...
    def enable_line_edit_file_drop(self, line_edit: QLineEdit):
        ...
        line_edit.dropEvent = lambda event: line_edit.setText(
            str(pathlib.Path(event.mimeData().urls()[0].toLocalFile()).absolute())
        )
    ...
    def mutually_exclude_scale_ratio_resolution(self):
        if (
            self.output_width_spin_box.value() != 0
            or self.output_height_spin_box.value() != 0
        ):
            self.scale_ratio_double_spin_box.setDisabled(True)
        elif (
            self.output_width_spin_box.value() == 0
            and self.output_height_spin_box.value() == 0
        ):
    ...
    def mutually_exclude_frame_interpolation_stream_copy(self):
        if (
            ...
            self.ffmpeg_migrate_streams_output_options_copy_streams_check_box.setChecked(
                False
            )
            self.ffmpeg_migrate_streams_output_options_copy_streams_check_box.setDisabled(
                True
            )
        else:
            self.ffmpeg_migrate_streams_output_options_copy_streams_check_box.setChecked(
            ...
            while output_path.exists():
                if input_path.is_file():
                    output_path = input_path.parent / pathlib.Path(
                        f"{input_path.stem}_output_{output_path_id}{suffix}"
                    )
                elif input_path.is_dir():
                    output_path = input_path.parent / pathlib.Path(
                        f"{input_path.stem}_output_{output_path_id}"
                    )
                output_path_id += 1
    ...
    def select_config_file(self):
        config_file = self.select_file(
            "Select Config File", filter="(YAML files (*.yaml))"
        )
